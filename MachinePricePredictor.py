...
class MachinePricePredictor:
    ...
    def load_data(self, file_path):
        ...
        # read the dataset
        df = pd.read_csv(file_path, parse_dates=['Sales date'], dtype=dtype_mapping, low_memory=False)
    ...
    def preprocess_data(self, df, drop_cols = ['Sales ID'], save=True):
        ...
        df['MachineHours CurrentMeter'] = df['MachineHours CurrentMeter'].fillna(df['MachineHours CurrentMeter'].median())
        df.fillna({'Usage Band': 'Unknown', 'Enclosure': 'None or Unspecified'}, inplace=True)
        ...
        df['Usage Intensity'] = df['MachineHours CurrentMeter'] / (df['Machine Age'] + 1)
        df['Has Enclosure'] = df['Enclosure'].apply(lambda x: 1 if x != 'None or Unspecified' else 0)
        ...
        df['Product Class Description'] = df['Product Class Description'].astype(str).fillna('')
        ...
        df[['Product Type', 'Min Value', 'MaxValue']] = df['Product Class Description'].str.extract(pattern, expand=True) 
        ...
        df['Units'] = df['Product Class Description'].str.extract(r"(Horsepower|Lb Operating Capacity|Metric Tons|Ft Standard Digging Depth)", expand=False)
    ...
    def prepare_features(self, X):
        ...
        numeric_transformer = Pipeline([
            ('imputer', SimpleImputer(strategy='median')), # median is best than mean specially for skewed data
            ('scaler', StandardScaler())
        ])

        categorical_transformer = Pipeline([
            ('imputer', SimpleImputer(strategy='most_frequent')), # most frequent is best for categorical data and we have a lot of categorical data
    ...
    def evaluate_model(self, y_test, y_pred):
        ...
        print(f'Average % Error: {np.mean(np.abs((y_test - y_pred) / y_test)) * 100:.2f}%')
    ...
    def plot_prediction_distribution(self, y_test, y_pred):
        ...
        plt.plot(grouped['True'], grouped['avg_pred'], color='r', label='Average Prediction', linewidth=2)
        plt.fill_between(grouped['True'], 
    ...
    def visualize(self, prefeature=True):
        ...
        sns.histplot(data=sales_price, bins=bins, kde=True, label=f'Skewness: {skewness:.2f}')
